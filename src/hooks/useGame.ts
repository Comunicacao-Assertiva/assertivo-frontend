  ...
  switch (action.type) {
    ...
    case 'SELECT_CHOICE': {
      ...
      const newBreakdown = state.phaseBreakdown.map((d, i) =>
        i === state.phase ? [...d, { pts: choice.points, type: choice.type }] : d
      )
      return {
        ...state,
        answered: true,
        ...
        lives: choice.type === 'wrong' ? Math.max(0, state.lives - 1) : state.lives,
    ...
    case 'NEXT_SCENARIO': {
      ...
      return { ...state, scenario: nextScenario, answered: false, selectedChoice: null }
    }

    case 'NEXT_PHASE': {
      const nextPhase = state.phase + 1
      if (nextPhase >= 5) return { ...state, screen: 'final' }
      return { ...state, phase: nextPhase, scenario: 0, answered: false, selectedChoice: null, screen: 'phase-intro' }
    }
...
export function useGame() {
  ...
  const getClassification = useCallback((score: number) => {
    ...
    if (pct >= 0.47) return { title: 'Comunicador em Crescimento', trophy: '🥈' }
    return { title: 'Aprendiz em Comunicação', trophy: '🌱' }
  }, [])

  const finalClassification = getClassification(state.score)

  const startGame     = useCallback(() => { dispatch({ type: 'RESTART' }); dispatch({ type: 'SET_SCREEN', screen: 'phase-intro' }) }, [])
  ...
  return {
    state, phases, currentPhase, currentScenario,
    globalProgress, finalClassification,
    startGame, selectChoice, nextScenario, nextPhase, restart, submitScore,
  }
}
