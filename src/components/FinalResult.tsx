...
export function FinalResult({ state, phases, classification, onRestart, onSubmitScore }: FinalResultProps) {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center px-5 py-10 text-center">
      <div className="mb-4 animate-bounce text-6xl">{classification.trophy}</div>
      ...
      <div className="text-6xl font-black text-amber-400 leading-tight">{state.score}</div>
      <p className="mb-2 text-base text-white/35">/ 1500 pontos</p>
      ...
      <div className="mb-6 grid w-full max-w-sm grid-cols-3 gap-2.5">
        {[
          ...
          <div key={l} className="rounded-xl border border-white/10 bg-white/5 py-3 text-center">
      ...
      <div className="mb-6 w-full max-w-sm space-y-2">
        {phases.map((ph, i) => (
          <div key={ph.id} className="flex items-center justify-between rounded-xl bg-white/5 px-4 py-2.5 text-sm">
            <span className="text-left text-white/60">Fase {ph.number} — {ph.title}</span>
            <span className="font-black text-amber-400">{state.phaseScores[i]}/300</span>
          </div>
        ))}
      </div>
      ...
      {!submitted ? (
        <div className="mb-6 w-full max-w-sm rounded-2xl border border-white/10 bg-white/5 p-5">
          <p className="mb-3 text-sm font-bold text-white/70">Salvar pontuação no placar?</p>
          <input
            type="text"
        ...
        <p className="mb-6 text-sm font-bold text-green-400">✅ Pontuação salva no placar!</p>
      )}
      ...
      <div className="flex w-full max-w-sm flex-col gap-3">
        ...
        <button
          onClick={() => {
            const msg = `🎮 Joguei "Assertivo! – O Jogo" e fiz ${state.score}/1500 pts!\n🏆 ${classification.title}\n\n"Seguindo a verdade em amor" — Ef 4:15`
            navigator.clipboard?.writeText(msg).then(() => alert('Resultado copiado!'))
          }}
