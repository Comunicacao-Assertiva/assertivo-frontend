...
export function PhaseResult({ phase, state, onNext, isLast }: PhaseResultProps) {
  ...
  const stars = phaseScore === 300 ? '⭐⭐⭐' : phaseScore >= 150 ? '⭐⭐' : phaseScore > 0 ? '⭐' : '—'
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center px-5 py-10 text-center">
      ...
      <p className="mb-1 text-sm text-white/40">Fase {phase.number} concluída!</p>

      <div className="mb-1 text-5xl font-black text-amber-400">{phaseScore}</div>
      <p className="mb-1 text-sm text-white/40">pontos nesta fase</p>
      <p className="mb-6 text-sm text-white/50">Total acumulado: <span className="font-bold text-white">{state.score} pts</span></p>

      {/* Breakdown */}
      <div className="mb-8 w-full max-w-sm rounded-2xl border border-white/10 bg-white/5 p-5">
        {[
          { label: 'Respostas assertivas (100 pts)', count: correct, pts: correct * 100 },
          { label: 'Respostas razoáveis (50 pts)', count: partial, pts: partial * 50 },
          { label: 'Respostas inadequadas (0 pts)', count: wrong, pts: 0 },
        ].map((row) => (
          <div key={row.label} className="flex justify-between border-b border-white/10 py-2 text-sm text-white/60 last:border-0">
            <span>{row.label}</span>
        ...
        <div className="flex justify-between pt-2 text-sm font-black">
          ...
          <span className="text-amber-400">{phaseScore}/300 pts</span>
        </div>
      </div>
