...
interface Props {
  topic: Topic; sub: SubPhase
  isNewTopic: boolean
  onStart: () => void
}

export function PhaseIntro({ topic, sub, isNewTopic, onStart }: Props) {
  const subNumber = topic.subPhases.indexOf(sub) + 1
    ...
    <div className="flex min-h-screen flex-col items-center justify-center px-5 py-10 text-center">
      ...
      {isNewTopic ? (
        <>
          ...
          <p className="mb-6 max-w-xs text-sm leading-relaxed text-white/55">{topic.description}</p>
          <div className="mb-6 flex gap-2">
            {[1,2,3].map(i => (
              <div key={i} className="rounded-full bg-white/10 px-3 py-1 text-xs font-bold text-white/40">
                Módulo {i}
              </div>
            ))}
          </div>
        ...
        <>
          <div className="mb-3 flex items-center gap-2 text-white/40 text-sm font-bold">
      ...
      <div className="mb-6 rounded-2xl border border-white/10 bg-white/5 px-5 py-4 max-w-sm w-full">
        ...
        <p className="text-sm text-white/55 leading-relaxed">{sub.description}</p>
      </div>
